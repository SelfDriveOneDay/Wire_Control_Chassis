...
#		if LCF_DEFAULT_HOST == LCF_CPU2
		group a0 (ordered, contiguous, align = 8, attributes=rw, run_addr = mem:dsram2)
#		endif
#		if LCF_DEFAULT_HOST == LCF_CPU1
		group a0 (ordered, contiguous, align = 8, attributes=rw, run_addr = mem:dsram1)
#		endif
#		if LCF_DEFAULT_HOST == LCF_CPU0
		group a0 (ordered, contiguous, align = 8, attributes=rw, run_addr = mem:dsram0)
#		endif
		{
			select "(.sdata |.sdata.*)";
			select "(.sbss |.sbss.*)";
		...
		/*Far data sections*/		
		group (ordered, contiguous, align = 8, run_addr = mem:dsram2)
		{
			select "(.data.data_cpu2|.data.data_cpu2.*)";
			select "(.bss.bss_cpu2|.bss.bss_cpu2.*)";
		}
		
		group (ordered, contiguous, align = 8, run_addr = mem:dsram1)
		{
			select "(.data.data_cpu1|.data.data_cpu1.*)";
			select "(.bss.bss_cpu1|.bss.bss_cpu1.*)";
		}
		
		group (ordered, contiguous, align = 8, run_addr = mem:dsram0)
		{
...
/*Heap sections*/
#		if LCF_DEFAULT_HOST == LCF_CPU2
		group (ordered, align = 8, run_addr = mem:dsram2[LCF_HEAP2_OFFSET])
#		endif
#		if LCF_DEFAULT_HOST == LCF_CPU1
		group (ordered, align = 8, run_addr = mem:dsram1[LCF_HEAP1_OFFSET])
#		endif
#		if LCF_DEFAULT_HOST == LCF_CPU0
		group (ordered, align = 8, run_addr = mem:dsram0[LCF_HEAP0_OFFSET])
#		endif
	    {
